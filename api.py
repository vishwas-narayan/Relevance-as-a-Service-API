...
class Api:
	...
	payload - a dictionary which is used to pass the parameters to a perticular URL query string.
	r - a response object which is used to GET data from the URL query string.
	data - a json decoder for the stream of data from r. 
	extract - It is used to get perticular lists(details of certain key attributes) from the dictionary
    ...
    def articles(self,query,page,rows):
    	...
    	It’s paginated and supports both relevance and time sorting orders.
